...
def get_timetable(chat_id):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, day, time, subject FROM timetable WHERE chat_id=%s ORDER BY day, time",
        (chat_id,)
    )
    rows = cur.fetchall()
    cur.close(); conn.close()
...
def get_tasks(chat_id):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, type, subject, due_date, done FROM tasks WHERE chat_id=%s ORDER BY due_date",
        (chat_id,)
    )
    rows = cur.fetchall()
    cur.close(); conn.close()
...
page = st.sidebar.radio("📂 Navigate", ["📅 Timetable", "📌 Tasks", "👥 All Users"])
        ...
        for day in DAYS:
            ...
            if day_rows:
                ...
                for r in day_rows:
                    ...
                    col1.write(f"🕐 `{r[2]}`")
                    col2.write(f"**{r[3]}**")
                    if col3.button("🗑️ Delete", key=f"del_tt_{r[0]}"):
                        delete_class_db(r[0])
                        st.success("Deleted!")
                        st.rerun()
    else:
        st.info("No timetable found. Add classes below or import via the bot.")
...
# ── Tasks ─────────────────────────────────────────────
    ...
    if pending:
        for r in pending:
            ...
            if col4.button("✅ Done", key=f"done_{r[0]}"):
                mark_done_db(r[0])
                st.rerun()
            if col5.button("🗑️", key=f"del_{r[0]}"):
                delete_task_db(r[0])
                st.rerun()
    else:
    ...
    col1, col2, col3, col4 = st.columns([2, 2, 2, 1])
    type_in = col1.selectbox("Type", ["assignment", "test"], key="task_type")
    subj_in = col2.text_input("Subject", placeholder="Physics", key="task_subj")
    date_in = col3.date_input("Due Date", key="task_date")
    if col4.button("➕ Add", key="task_add_btn"):
        if subj_in:
            add_task_db(chat_id, type_in, subj_in, str(date_in))
            st.success(f"✅ Added {type_in}: {subj_in} due {date_in}!")
            st.rerun()
        else:
...
# ── Users ─────────────────────────────────────────────
    ...
    for u in users:
        ...
        col1.write(f"👤 **{u[1]}**")
        col2.write(f"`{u[0]}`")
        col3.write(f"Joined: {str(u[2])[:10]}")
